...
export class UploadService {
	...
	async upload(file: Express.Multer.File): Promise<{
		...
		const quotesInput = uploadContentDto.quotes?.filter((i) => i.content && (i.author || i.authorId)) ?? []
		...
		return await getClient().transaction(async (t) => {
			const createAuthorResult = await this.authorService.bulkCreate(authorsInput, {
				transaction: t,
			})

			const createQuoteResult = await this.quoteService.bulkCreate(quotesInput, {
				transaction: t,
			})
