...
import { AuthorDto, CreateAuthorDto, IndexAuthorsDto, UpdateAuthorDto } from './author.dto'
...
export class AuthorService {
	...
	async getByShortId(id: string) {
		const author = await this.authorRepository.getByShortId(id, {
			findOrFail: true,
		})
	...
	async getByIds(ids: UUIDTypes[], options: { transaction?: Transaction } = {}) {
		return await this.authorRepository.getByIds(ids, options)
	}

	async getBySlugs(slugs: string[], options: { transaction?: Transaction } = {}) {
